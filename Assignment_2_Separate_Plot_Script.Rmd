...
```{r}
library(tidyverse)  ## BMB includes ggplot2, dplyr, tidyr ... so those are redundant
library(ggplot2)
library(respR)
library(dplyr)
library(tidyr)
...
```{r}
background_abs<-background_B4$rate.abs ## BMB: don't need quotation marks unless name has spaces etc.
total_abs<-mean(total_A1$rate.abs)
fish_abs<-(total_abs-background_abs)
```
**BMB**: these calculations should usually be done **within** a data frame to make sure variables are properly aligned

#calculating the fish resting metabolic rate of the fish alone
```{r}
print(fish_RMR<-mean(abs((fish_abs)/(total_A1$"mass")))) #units umolO2/min/kg
```

**BMB**: I don't understand a lot of this automated analysis, and the level of automation makes me a little nervous (since I don't understand it/know what the assumptions are/wouldn't know how to diagnose or adjust it) - I hope you do ...
